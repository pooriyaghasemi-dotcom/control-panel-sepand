...
// ============ تنظیمات WiFi ============
const char* AP_SSID = "ESP32_YARD_AP";
const char* AP_PASSWORD = "12345678";

char wifi_ssid[32] = "";
char wifi_password[64] = "";
char device_static_ip[16] = "192.168.1.102";
...
#define DOOR1_SWITCH_PIN 5  // درب ورودی 1
#define DOOR2_SWITCH_PIN 6  // درب ورودی 2
#define LASER1_PIN 16       // لیزر 1
#define LASER2_PIN 17       // لیزر 2
#define LASER3_PIN 18       // لیزر 3
...
// ============ توابع سنسورهای لیزری ============
bool getLaserStatus(int laser_id) {
  int pin = LASER1_PIN;
  if (laser_id == 2) pin = LASER2_PIN;
  else if (laser_id == 3) pin = LASER3_PIN;
  
  return digitalRead(pin) == HIGH; // اگر پرتو قطع شود، HIGH می‌شود
}
...
void sendDataToServer() {
  ...
  doc["door1_open"] = digitalRead(DOOR1_SWITCH_PIN) == HIGH;
  doc["door2_open"] = digitalRead(DOOR2_SWITCH_PIN) == HIGH;
  
  // وضعیت لیزرها
  JsonArray lasers = doc.createNestedArray("lasers");
  for (int i = 1; i <= 3; i++) {
    JsonObject laser = lasers.createNestedObject();
    laser["id"] = i;
    laser["triggered"] = getLaserStatus(i);
  }
...
void setup() {
  ...
  Serial.println("\n\n===== Yard Client Starting =====");
  
  // تنظیم پین‌ها
  pinMode(DOOR1_SWITCH_PIN, INPUT_PULLUP);
  pinMode(DOOR2_SWITCH_PIN, INPUT_PULLUP);
  pinMode(LASER1_PIN, INPUT);
  pinMode(LASER2_PIN, INPUT);
  pinMode(LASER3_PIN, INPUT);
